...
struct ubnt_bd_t ubnt_bd_g;
EXPORT_SYMBOL(ubnt_bd_g);
#define UBNT_BD_MTD_PART	4
...
static int read_ubnt_bd(struct ubnt_bd_t *ubnt_bd)
{
	struct mtd_info *mtd;
	char buf[DR_LEN+BD_LEN];
	int len;
	u16 crc16;

	mtd = get_mtd_device(NULL, UBNT_BD_MTD_PART);
	if (IS_ERR(mtd)) {
		printk("Failed to read MTD%d\n", UBNT_BD_MTD_PART);
		return -1;
	}
	if (mtd_read(mtd, 0, DR_LEN+BD_LEN, &len, buf) != 0) {
		printk("read mtd failed\n");
		return -1;
	}
	memcpy(ubnt_bd, buf+DR_LEN, BD_LEN);
        GenCrcTable();
	crc16 = calc_crc(((u8 *)ubnt_bd) + 2, BD_LEN - 2 - 2);
	if (crc16 == ubnt_bd->crc16) {
		printk("UBNT BD mac %02x%02x%02x%02x%02x%02x kidx %d mrev %d serial %s type %s\n",
		ubnt_bd->mac[0],ubnt_bd->mac[1],ubnt_bd->mac[2],ubnt_bd->mac[3],ubnt_bd->mac[4],
		ubnt_bd->mac[5], ubnt_bd->kidx, ubnt_bd->mrev, ubnt_bd->serial, ubnt_bd->type);
	} else {
		printk("UBNT BD CRC fail\n");
        	get_random_bytes(ubnt_bd->mac, 6);
		ubnt_bd->mac[0] &= 0xfe; /* clear multicast bit */
		ubnt_bd->mac[0] |= 0x02; /* set local assignment bit (IEEE802) */
	}
	return 0;
}
