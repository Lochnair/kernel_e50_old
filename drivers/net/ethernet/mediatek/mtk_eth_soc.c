	...
	u32 tx_pkt, u32 tx_byte, u32 tx_err, u32 tx_drop)
{
	...
	stats->rx_packets += rx_pkt;
	stats->rx_bytes += rx_byte;
...
void __iomem *fe_sysctl_base;
EXPORT_SYMBOL(fe_sysctl_base);
void __iomem *fe_base;
EXPORT_SYMBOL(fe_base);
...
static int __init fe_init(struct net_device *dev)
{
	...
	if (mac_addr)
		...
		memcpy(dev->dev_addr, ubnt_bd_g.mac, 6);
...
{
	//UBNT - Add MT7530 and RTL8367 support
	struct fe_priv *priv = netdev_priv(dev);
	...
	case RAETH_MII_READ:
		...
		mii.val_out = mt7530_mdio_r32(gsw_mt7621, mii.reg_num);
		result = copy_to_user(ifr->ifr_data, &mii, sizeof(mii));
		//printk("%s: mii.val_out=0x%08X\n", __func__, mii.val_out);
		return 0;
		break;
	...
	case RAETH_MII_WRITE:
		...
		mt7530_mdio_w32(gsw_mt7621, mii.reg_num, mii.val_in);
		if(mii.reg_num == 0x90) {
			for (i = 0; i < 20; i++) {
				mii.val_out = mt7530_mdio_r32(gsw_mt7621, 0x90);
				if ((mii.val_out & 0x80000000) == 0 ){ //table busy
					break;
				}
				usleep_range(1000, 1100);
			}
			if (i == 20)
				printk(KERN_ERR "%s: ACL table busy - timeout.\n", __func__);
		}
		//printk("%s: mii.val_in=0x%08X\n", __func__, mii.val_in);
		return 0;
		break;
	...
	while (entry < end) {
		if (IS_IPV4_HNAPT(entry)) {
			...
			*((u16*) &h_dest[4]) = swab16(entry->ipv4_hnapt.dmac_lo);
			...
			printk(KERN_ERR "0x%05x state=%s\n",
