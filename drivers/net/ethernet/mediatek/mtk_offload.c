...
// UBNT: These are for ER-10X reference. 
// Marked because MT7530 HW limited 64 accounting groups, so not support RTL8367 yet.
/*
#define PORT_NUM 10
		   ...
		   struct flow_offload_hw_path *dest)
{
	...
	if (src_port < AG_INPUT_NUM && dst_port < AG_OUTPUT_NUM)
		entry->ipv4_hnapt.iblk2.port_ag = ag_map[src_port][dst_port];
	else
		entry->ipv4_hnapt.iblk2.port_ag = 0;
...
{
	int in, out, i;
	uint32_t ag_sum[AG_OUTPUT_NUM][2][2]; // [rx,tx][pkt,byte]
	...
	for (i = 0; i < PORT_NUM; i++) {
		// switched ports counters are already updated by platform module
		extern int mt7621_is_switch0_member(u32 port);
		if ( mt7621_is_switch0_member(i) == 1 )
			continue;
		upd_eth_stats(i, ag_sum[i][0][0], ag_sum[i][0][1], 0, 0,
