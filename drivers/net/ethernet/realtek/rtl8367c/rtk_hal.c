...
int rtk_hal_set_svlan(struct ra_switch_ioctl_data *data)
{
	...
	for (i = 0; i < 5; i++) {
		if (data->port_map & (1 << i)) {
			RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, i);
		}
	}
	for (i = 0; i < 2; i++) {
		if (data->port_map & (1 << (i + 5))) {
			RTK_PORTMASK_PORT_SET(svlan_cfg.memberport, (i + EXT_PORT0));
		}
	}

	for (i = 0; i < 5; i++) {
		if (data->untag_map & (1 << i)) {
			RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, i);
		}
	}
	for (i = 0; i < 2; i++) {
		if (data->untag_map & (1 << (i + 5))) {
			RTK_PORTMASK_PORT_SET(svlan_cfg.untagport, (i + EXT_PORT0));
		}
	}
	
	rtk_svlan_memberPortEntry_set(data->vid, &svlan_cfg);
...
{
	rtk_svlan_defaultSvlan_set(data->port, data->vid);
	return 0;
}

int rtk_hal_set_svlan_untag(struct ra_switch_ioctl_data *data)
{
	rtk_svlan_untag_action_set(UNTAG_ASSIGN, data->vid);
	return 0;
}

int rtk_hal_set_svlan_unmatch(struct ra_switch_ioctl_data *data)
{
	rtk_svlan_unmatch_action_set(UNMATCH_ASSIGN, data->vid);
	return 0;
}
...
int rtk_hal_dump_vlan(void)
{
	...
	for (i = 0; i < 4096; i++) {
		rtk_vlan_get(i, &vlan);
		//UBNT_Andrew 2018/04/24 - Add handler RTK_SW_VID_RANGE to 4095
		if((i < RTK_SW_VID_RANGE) || (RTK_PORTMASK_IS_EMPTY(vlan.mbr) != 1)) {
			printk("%4d    ", i);
			printk("%c",
...
int rtk_hal_set_pvid(struct ra_switch_ioctl_data *data)
{
	rtk_api_ret_t ret;
	...
	memset(&vlan, 0x00, sizeof(rtk_vlan_cfg_t));
	rtk_vlan_get(data->vid, &vlan);

	/* clear vlan entry first */
	memset(&pVlanCfg, 0x00, sizeof(rtk_vlan_cfg_t));
	RTK_PORTMASK_CLEAR(pVlanCfg.mbr);
	RTK_PORTMASK_CLEAR(pVlanCfg.untag);
	rtk_vlan_set(data->vid, &pVlanCfg);
	memset(&pVlanCfg, 0x00, sizeof(rtk_vlan_cfg_t));
	
	ret = rtk_vlan_portPvid_set(data->port, data->vid, 0);
	rtk_vlan_set(data->vid, &vlan);

	return ret;
}

int rtk_hal_set_vlan_tag(struct ra_switch_ioctl_data *data)
{
	rtk_vlan_cfg_t vlan, pVlanCfg;
	rtk_api_ret_t ret;
...
int rtk_hal_table_clear(struct ra_switch_ioctl_data *data)
{
	rtk_api_ret_t ret;
...
int rtk_hal_set_tagmode(struct ra_switch_ioctl_data *data)
{
	rtk_api_ret_t ret;

	ret = rtk_vlan_tagMode_set(data->port, data->mode);

	return 0;
}

int rtk_hal_set_transparent(struct ra_switch_ioctl_data *data)
{
	rtk_portmask_t pIgr_pmask;
	rtk_api_ret_t ret;
	int i;

	RTK_PORTMASK_CLEAR(pIgr_pmask);

	for (i = 0; i < 5; i++) {
		if (data->port_map & (1 << i)) {
			RTK_PORTMASK_PORT_SET(pIgr_pmask, i);
		}
	}
	for (i = 0; i < 2; i++) {
		if (data->port_map & (1 << (i + 5))) {
			RTK_PORTMASK_PORT_SET(pIgr_pmask, (i + EXT_PORT0));
		}
	}

	ret = rtk_vlan_transparent_set(data->port, &pIgr_pmask);
